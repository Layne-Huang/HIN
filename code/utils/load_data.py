...
def is_every_row_non_zero(matrix):
    # 遍历矩阵的每一行，检查是否包含非零元素
    for row in matrix:
        if np.all(row == 0):
            return False
...
def load_acm(ratio, type_num):
    ...
    nei_s = np.load(path + "nei_s.npy", allow_pickle=True) # 4019x1 # the subject neighbors of paper as each paper has only one
    feat_p = sp.load_npz(path + "p_feat.npz") # 4019x1902
    feat_a = sp.eye(type_num[1]) # 7167x7167
    feat_s = sp.eye(type_num[2]) # 60x60
    pap = sp.load_npz(path + "pap.npz") # 4019x4019 meta path pf paper-author-paper
    psp = sp.load_npz(path + "psp.npz") # 4019x4019 meta path of paper-subject-paper
    pos = sp.load_npz(path + "pos.npz")  # 4019x4019 positive 
    row_max_values = np.max(pos, axis=1)
    # print(is_every_row_non_zero(pos.toarray()))
    train = [np.load(path + "train_" + str(i) + ".npy") for i in ratio] # 3 item
    test = [np.load(path + "test_" + str(i) + ".npy") for i in ratio] #3 item
...
def load_huya(ratio, type_num):
    ...
    feat_a = sp.load_npz(path + "feat_a.npz") 
    feat_i = sp.eye(type_num[1])
    feat_t = sp.eye(type_num[2]) 
    aia = sp.load_npz(path + "aia.npz")
    ...
    aia = sparse_mx_to_torch_sparse_tensor(normalize_adj(aia))
    ata = sparse_mx_to_torch_sparse_tensor(normalize_adj(ata))
    pos = sparse_mx_to_torch_sparse_tensor(pos)
    train = [th.LongTensor(i) for i in train]
    val = [th.LongTensor(i) for i in val]
    test = [th.LongTensor(i) for i in test]
    return [nei_i, nei_t], [feat_a, feat_i, feat_t], [aia, ata], pos, label, train, val, test
